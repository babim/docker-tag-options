...
# visible code
	if check_value_true "${VISIBLECODE}"; then
		...
		# 	echo 'JAVA_OPTS="-javaagent:${SOFT_INSTALL}/agent.jar ${JAVA_OPTS}"' >> "${SOFT_INSTALL}/bin/start_${SOFT}.sh"
		# elif [[ -f  "${SOFT_INSTALL}/start-${SOFT}.sh" ]]; then
		# 	echo 'JAVA_OPTS="-javaagent:${SOFT_INSTALL}/agent.jar ${JAVA_OPTS}"' >> "${SOFT_INSTALL}/start-${SOFT}.sh"
		# elif [[ -f  "${SOFT_INSTALL}/start_${SOFT}.sh" ]]; then
		# 	echo 'JAVA_OPTS="-javaagent:${SOFT_INSTALL}/agent.jar ${JAVA_OPTS}"' >> "${SOFT_INSTALL}/start_${SOFT}.sh"
		# elif [[ -f  "${SOFT_INSTALL}/bin/start.sh" ]]; then
		# 	echo 'JAVA_OPTS="-javaagent:${SOFT_INSTALL}/agent.jar ${JAVA_OPTS}"' >> "${SOFT_INSTALL}/bin/start.sh"
		# elif [[ -f  "${SOFT_INSTALL}/start.sh" ]]; then
		# 	echo 'JAVA_OPTS="-javaagent:${SOFT_INSTALL}/agent.jar ${JAVA_OPTS}"' >> "${SOFT_INSTALL}/start.sh"
		# fi
	else
