...
## download and extract source software
	...
	if [[ "$SOFTSUB" == "servicedesk" ]]; then
		check_folder_empty "${SOFT_INSTALL}" && curl -Ls "https://www.atlassian.com/software/${SOFT}/downloads/binary/atlassian-${SOFTSUB}-${SOFT_VERSION}.tar.gz" | tar -xz --directory "${SOFT_INSTALL}" --strip-components=1 --no-same-owner
	else
		check_folder_empty "${SOFT_INSTALL}" && curl -Ls "https://www.atlassian.com/software/${SOFT}/downloads/binary/atlassian-${SOFT}-${SOFTSUB}-${SOFT_VERSION}.tar.gz" | tar -xz --directory "${SOFT_INSTALL}" --strip-components=1 --no-same-owner
	fi
...
## set permission path
	...
	FILETEMP="${SOFT_INSTALL}/bin/check-java.sh"
		say "sed ${FILETEMP}..."	
		check_file "${FILETEMP}"	&& sed --in-place "s/java version/openjdk version/g" "${FILETEMP}" || say_warning "${FILETEMP} does not exist"
# add soft home path
...
if [[ -f /etc/alpine-release ]]; then
	...
	# install depend
		...
		install_package curl xmlstarlet ttf-dejavu tzdata fontconfig msttcorefonts-installer
		update-ms-fonts
