...
EOF
	...
	if [[ ${MACHINE_TYPE} == 'x86_64' ]]; then
		if [[ ${FIXED} == 'true' ]]; then
			curl -Ls http://file.matmagoc.com/ManageEngine/ManageEngine_SupportCenter_Plus_64bit.bin -o $FILE_TEMP
		else
		...
		if [[ ${FIXED} == 'true' ]]; then
			curl -Ls http://file.matmagoc.com/ManageEngine/ManageEngine_SupportCenter_Plus.bin -o $FILE_TEMP
		else
			curl -Ls https://www.manageengine.com/products/support-center/64045241/ManageEngine_SupportCenter_Plus.bin -o $FILE_TEMP
		fi
	else
