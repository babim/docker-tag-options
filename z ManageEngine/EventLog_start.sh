...
EOF
	...
	if [[ ${MACHINE_TYPE} == 'x86_64' ]]; then
		keystroke
		if [[ ${FIXED} == 'true' ]]; then
			$download_save install.bin http://media.matmagoc.com/ManageEngine/ManageEngine_EventLogAnalyzer_64bit.bin
		else
		...
		if [[ ${FIXED} == 'true' ]]; then
			...
			$download_save install.bin https://www.manageengine.com/products/eventlog/91517554/ManageEngine_EventLogAnalyzer.bin
		fi
	else
