...
EOF
	...
	if [[ ${MACHINE_TYPE} == 'x86_64' ]]; then
		keystroke
		if [[ ${FIXED} == 'true' ]]; then
			$download_save install.bin http://media.matmagoc.com/ManageEngine/ManageEngine_KeyManagerPlus_64bit.bin
		else
			$download_save install.bin https://www.manageengine.com/key-manager/97531/ManageEngine_KeyManagerPlus_64bit.bin
		fi
	elif [[ ${MACHINE_TYPE} != 'x86_64' ]]; then
		opmanagerkeystroke
		if [[ ${FIXED} == 'true' ]]; then
			$download_save install.bin http://media.matmagoc.com/ManageEngine/ManageEngine_KeyManagerPlus.bin
		else
			$download_save install.bin https://www.manageengine.com/key-manager/97531/ManageEngine_KeyManagerPlus.bin
		fi
	else
